...
import com.taskify.todomanager.data.workmanager.TaskReminderWorker
...
class TaskRepository(
    ...
    override fun insertTask(task: TaskEntity): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to add task: ${e.message}"))
        }
    }
    ...
    override fun updateTask(task: TaskEntity): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to update task.: ${e.message}"))
        }
    }
    ...
    override fun deleteTask(task: TaskEntity): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to delete task: ${e.message}"))
        }
    }
    ...
    override fun getAllTasks(): Flow<Resource<List<TaskEntity>>> = flow {
        try {
            ...
            emit(Resource.Error("Failed to fetch task list: ${e.message}"))
        }
    }
    override fun getPriorityTasks(): Flow<Resource<List<TaskEntity>>> = flow {
        try {
            emit(Resource.Loading())
            val result = taskDao.getTasksByPriority()
            emit(Resource.Success(result))
        } catch (e: Exception) {
            emit(Resource.Error("Failed to fetch task list: ${e.message}"))
        }
    ...
    override fun searchTasks(query: String): Flow<Resource<List<TaskEntity>>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to search tasks: ${e.message}"))
        }
    }
    ...
    override fun filterTasksByCategory(categoryId: Int): Flow<Resource<List<TaskEntity>>> = flow {
        try {
            ...
            emit(Resource.Error("Failed to filter tasks: ${e.message}"))
        }
    }
    ...
    override fun pinTask(taskId: Int, isPinned: Boolean): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to pin task: ${e.message}"))
        }
    }
    ...
    override fun markAsDone(taskId: Int, isDone: Boolean): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to mark task as completed"))
        }
    }
    ...
    override fun multipleMarkAsDone(taskIds: List<Int>): Flow<Resource<Unit>> = flow {
        try {
            ...
            emit(Resource.Error("Failed to mark task as completed"))
        }
    }
    ...
    override fun getTaskById(taskId: Int): Flow<Resource<TaskEntity>> = flow {
        try {
            ...
            emit(Resource.Error("Unable to load tasks"))
        }
    }
}
